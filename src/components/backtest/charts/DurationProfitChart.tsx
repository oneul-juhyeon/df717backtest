...
import { DurationProfitDataPoint } from "@/hooks/useBacktestData";

interface Props {
  data: DurationProfitDataPoint[];
}
...
const DurationProfitChart = ({ data }: Props) => {
  const [tooltipData, setTooltipData] = useState<{
    x: number;
    y: number;
    dataPoint: DurationProfitDataPoint;
    chartWidth: number;
    chartHeight: number;
  } | null>(null);
    ...
    <div className="w-full h-[274px] relative">
      ...
      {tooltipData && (() => {
        ...
        return (
          ...
          >
            <p className="text-sm text-foreground font-bold mb-1">{dataPoint.duration}</p>
            <p className={`font-mono text-xs ${isPositive ? 'text-success' : 'text-destructive'}`}>
              {isPositive ? '+' : ''}{formatCurrency(dataPoint.profit)}
            </p>
            <p className="font-mono text-xs text-muted-foreground">{dataPoint.trades} trades</p>
          </div>
        );
      })()}
    </div>
...
export default DurationProfitChart;
