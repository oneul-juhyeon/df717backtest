...
import { DayProfitDataPoint } from "@/hooks/useBacktestData";

interface Props {
  data: DayProfitDataPoint[];
}
...
const ProfitByDayChart = ({ data }: Props) => {
  const [tooltipData, setTooltipData] = useState<{
    x: number;
    y: number;
    dataPoint: DayProfitDataPoint;
    chartWidth: number;
    chartHeight: number;
  } | null>(null);
    ...
    <div className="w-full h-[274px] relative">
      ...
      {tooltipData && (() => {
        ...
        return (
          ...
          >
            <p className="text-sm text-foreground font-bold mb-1">{dataPoint.day}</p>
